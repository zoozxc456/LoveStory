  ...
  <CommonDropDownList v-model:display-controller="displayController">
    <template #presentation>
      <input
        ...
        :value="model.map((x) => x.specialNeedContent).join(',')"
      />
    </template>
    <template #list>
      <div
      ...
      >
        <div class="py-1" role="none">
          ...
          >
            <div>{{ item }}</div>
            <div
              v-if="model.map((x) => x.specialNeedContent).includes(item)"
...
const model = defineModel<IGuestSpecialNeed[]>("specialNeeds", {
  required: true,
});
...
const handleSelectItem = (item: string) => {
  const itemIndex = model.value.findIndex((x) => x.specialNeedContent === item);
