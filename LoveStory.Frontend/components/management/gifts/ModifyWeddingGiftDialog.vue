...
const formData = defineModel<ModifyWeddingGiftFormData>("formData", {
  required: true,
});

const handlers = {
  onClickModifyButton: () => {
    useWeddingGiftManagementStore().modifyWeddingGift(formData.value, () => {
      emits("modified");
      handlers.onCancel();
    });
  },
